...
SELECT name,salary FROM employees e WHERE e.salary > 60000;

-- Question 2: Find Total Sales Per Customer 
-- Calculate the total sales amount for each customer from the sales table.
SELECT customer_id, Sum(amount) AS total_sales_amount FROM sales s GROUP BY s.customer_id;
...
SELECT sale_date, Sum(amount) As total_sales_amount from sales Where sale_date = '2023-03-17';

-- Question 5: High-Value Orders
-- Get the names of customers who have placed an order of more than 600.
SELECT customer_name, order_amount FROM orders o WHERE o.order_amount > 600;
